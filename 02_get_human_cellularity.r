...
colnames(human_cellularities) <- c("human_name", "human_cellularity")
human_names <- strsplit(reports, split = "/")
...
for (j in 1:length(reports)) {
	...
	human_cellularities[j,] <- c(human_names[j], print(human_cellularity))
}
